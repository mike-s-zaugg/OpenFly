...
if (args.has("readout")) {
  FlyRegistry.setReadout(new Readout(JSON.parse(fs.readFileSync(args.get("readout")!, "utf8")) as ReadoutWeights));
}
...
console.log(JSON.stringify({ ...res, actionCounts: Object.fromEntries(ACTIONS.map((a, i) => [a.key, res.actionCounts[i]])) }, null, 1));
